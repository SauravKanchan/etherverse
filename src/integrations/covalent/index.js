...
export async function fetchTokenDetails(address, chainId) {
    ...
    for (let item of tokens){
        ...
        {
            returnObj[item.contract_ticker_symbol] = ethers.utils.formatUnits( item.balance, item.contract_decimals);
            returnObj[item.contract_ticker_symbol] = (+returnObj[item.contract_ticker_symbol]).toFixed(4);
        }
    }
